...
%
%%%%NOTES:
%   It is recommended to only use time series of intra-cranial voxels as
...
elseif isempty(scl) && isempty(md)    
    md = mean(mean(Y,2)); %added the next 2 lines to perform grand mean scaling, if removed no scaling/norm is performed
    scl = 100;
    Y   = IntnstyScl(Y,md,scl);
           if verbose;  disp('-Set to grand mean scaling'); end;
